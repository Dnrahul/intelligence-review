...
BROWSER_PROFILES = ["chrome110", "chrome116", "chrome120", "chrome124", "chrome99", "chrome100", "chrome101", "chrome102", "chrome103", "chrome104", "chrome105", "chrome106", "chrome107", "chrome108", "chrome109", "edge99", "edge100", "edge101", "edge102", "edge103", "edge104", "edge105", "edge106", "edge107", "edge108", "edge109", "edge110", "edge111", "edge112", "edge113", "edge114", "edge115", "edge116", "edge117", "edge118", "edge119", "edge120", "edge121", "edge122", "edge123", "edge124"]
...
def _detect_platform(url: str) -> str:
    ...
    if "amazon" in domain or "amzn.in" in domain:
        return "amazon"
...
class ReviewScraper:
    ...
    def _is_blocked(self, html: str) -> bool:
        ...
        blocked_indicators = [
            ...
            "Sign in or create account",  # Amazon login page
            "Enter mobile number or email",  # Amazon login form
        ]
    ...
    async def _scrape_amazon(
        ...
        # Resolve redirects for short links
        resolved_url = await self._resolve_redirect(url)

        # First, try to get product info and reviews from the product page itself
        try:
            product_soup = await self._fetch_page(resolved_url)
            product_name, product_image = self._extract_amazon_product_info(product_soup)
            
            # Try to scrape reviews from product page
            review_cards = self._find_amazon_review_cards(product_soup)
            for card in review_cards:
                if len(reviews) >= max_reviews:
                    break
                try:
                    review = self._parse_amazon_review(card)
                    if review and not any(r.id == review.id for r in reviews):
                        reviews.append(review)
                except Exception as e:
                    logger.warning(f"Failed to parse review from product page: {e}")
        except Exception as e:
            logger.warning(f"Could not fetch product page: {e}")

        # Convert product URL to reviews URL
        reviews_url = self._amazon_reviews_url(resolved_url)
    ...
    def _amazon_reviews_url(self, url: str) -> str:
        ...
        asin = self._extract_amazon_asin(resolved_url)
        if asin:
